program ThreePCF
  ...
  real :: avgal,avran
  real(kdkind) :: ndd,nrr
  integer :: k, i, j, l,m,n,d,chunk,nmu,nbins,iloop,nrel
...
print*,rmin
  do i=1,nbins
    if(logbins) then
      rbin(i)=10**(log10(rmin)+(log10(rmax)-log10(rmin))/nbins)
    else
      rbin(i)=rmin+i*(rmax-rmin)/float(nbins)
    endif
    print*, rbin(i)
  enddo 
...
!$OMP PARALLEL DO schedule(dynamic)  private(i,j,k,vec_dist,v1,v2,v3,v4,nn1,nn2,nnode,resultsb,theta,nrel) & 
!$OMP& shared(tree,Ndata,myid,output,odtheta,nmu,rmin,rmax,rbin)
do i=istart,iend
   nrel=0
...
!!!!!!NEW!!!!!!
    ...
    if(RSD) allocate(output(i)%mu(nnode))
!!!!!!NEW!!!!!!
    if(RSD) v2=my_array(:,i)


    do k=1,nbins

      nn2=kdtree2_r_count_around_point(tp=tree,idxin=i,correltime=-1,r2=rbin(k)*rbin(k))
      do j=nn1+1,nn2
        nrel=nrel+1
        output(i)%dist(nrel)=k
        output(i)%id(nrel)=resultsb(j)%idx
      enddo
      nn1=nn2
    enddo
...
!        else
!          output(i)%dist(k)=floor(vec_dist*odr)+1
!        endif
...
!!!!!!NEW!!!!!!
  if(RSD) then
    ...
    call sort3(output(i)%id,output(i)%dist,nnode)
  endif
!!!!!!NEW!!!!!!
    nn2=0
...
33      if (idum>nn2 .or. jdum>nn3) then
        ...
        if(RSD) mu2=output(nodeid)%mu(jdum)

...
the DESI DR1 LRG sample \citep{desi24} ($1.48$M galaxies NGC, $0.66$M
SGC; weights
...
1000) EZmock realisations \citep{chuang15} processed through the
identical pipeline.
...
Gpc})^3$ boxes at $z = 0.5$, with the fiducial cosmology
($\Omega_{\rm m} = 0.3175$, $\sigma_8 = 0.834$, $h = 0.6711$) and the
paired single-parameter variations $\Omega_{\rm m}^{\pm}$ ($\pm 0.010$),
$\sigma_8^{\pm}$ ($\pm 0.015$), and $h^{\pm}$ ($\pm 0.02$).  A fixed
number-density cut $\bar n = 1.5\times10^{-4}\,(h/{\rm Mpc})^{3}$ ---
the $150\,000$ most massive halos per box --- is applied identically to
every catalogue, so that parameter responses reflect changes in
clustering rather than abundance.  Randoms are uniform in the periodic
box at 1:1.  We measure the 3PCF in 20 radial bins over
$10$--$150\Mpch$ and the connected 4PCF in 5 bins to $65\Mpch$, for 100
fiducial realisations and 50 per variation: 400 realisations and
$1.0\times10^{6}$ binned configurations in total, completing in
${\sim}10$ hours on the single GPU (${\sim}50$\,s per 3PCF and
${\sim}9$\,s per 4PCF realisation).

\subsection{Fiducial measurement}

Figure~\ref{fig:qfid} shows the equilateral 3PCF of the fiducial
ensemble with box-to-box scatter: the expected hierarchical behaviour at
small scales, a shallow negative trough at $35$--$50\Mpch$, and
amplitudes consistent with zero at large separations.  Notably, no
...
\begin{figure}
\centering
\includegraphics[width=0.95\columnwidth]{figures/quijote_fiducial_3pcf.pdf}
\caption{Equilateral 3PCF of Quijote fiducial FoF halos at $z=0.5$
(number density matched to $1.5\times10^{-4}\,(h/{\rm Mpc})^{3}$),
averaged over 100 realisations.  Error bars show the box-to-box scatter
of a single $(1\,h^{-1}{\rm Gpc})^3$ realisation.
\label{fig:qfid}}
\end{figure}
...
equilateral 3PCF and 4PCF to each parameter pair,
...
adjustment can mimic.  The full measurements span all $1540$ (3PCF) and
$900$ (4PCF) configurations per realisation; the equilateral cuts shown
here are illustrative, and we emphasise that this is a qualitative
...
\begin{figure*}
\centering
\includegraphics[width=\textwidth]{figures/quijote_3pcf_responses.pdf}
\caption{Response of the equilateral 3PCF to the Quijote parameter
variations, in units of the single-realisation fiducial scatter.  Error
bars propagate the ensemble errors of the variation means (50
realisations per side).  The responses are confined to quasi-linear
scales, and the matched number density suppresses the $\sigma_8$
response relative to the shape parameters $\Omega_{\rm m}$ and $h$ (see
text).
\label{fig:q3resp}}
\end{figure*}
...
\begin{figure*}
\centering
\includegraphics[width=\textwidth]{figures/quijote_4pcf_responses.pdf}
